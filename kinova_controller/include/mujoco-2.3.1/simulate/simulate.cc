...
std::vector<double> EigenVec2Mat(Eigen::Vector3d z)
{
...
void Simulate::renderloop() {
  ...
  // run event loop
  while (!Glfw().glfwWindowShouldClose(this->window) && !this->exitrequest.load() && rclcpp::ok()) 
  {
    {
    ...
    #ifdef VISUALIZATION_PARTICLE
    const float deep_red_color[4] = {1,0,0,1}; // RED
    double arrow_size = 0.01; // arrow size

    //color
    const float pink_color[4]={1,204/255,1,0.3};    //for particle set 1
    const float yellow_color[4]={1.0, 1.0, 0.0, 0.3};
    const float deep_pink_color[4]={1,204/255,1,1};

    const float blue_color[4]={0,0,1,0.3}; // for particle set 2
    const float cyon_color[4]={0, 1, 1, 0.3};
    const float deep_blue_color[4]={0,0,1,0.5};

    const float green_color[4]={0,1,0,0.3}; // for particle set 3
    const float white_green_color[4]={204.0/255.0, 1, 204.0/255.0,0.3};
    const float deep_green_color[4]={0,1,0,0.5};

    const float brown_color[4]={102.0/255.0, 51.0/255.0, 0, 0.5}; // for particle set 4
    const float orange_color[4]={1, 153.0/255.0, 0, 0.5};
    const float deep_brown_color[4]={102.0/255.0, 51.0/255.0, 0, 1};

    const float white_color[4]={1,1,1,0.5}; // for else particle set
    const float gray_color[4]={102.0/255.0, 102.0/255.0, 102.0/255.0, 0.5};
    const float deep_white_color[4]={1,1,1,1};

    
    if(mcp_ep_visualization.is_visualize)
    {
      for(int j=0; j<mcp_ep_visualization.particle_pos.size(); j++)
      {   
          for(int i=0; i<mcp_ep_visualization.particle_pos.at(j).size(); i++)
          {

              mjvGeom* particle_force = scn.geoms + scn.ngeom++;

              switch(j)
              {
                  case 0 :
                  if(i>mcp_ep_visualization.particle_pos.at(j).size()-mcp_ep_visualization.exploit_particle_num.at(j))
                  {
                      mjv_initGeom(particle_force, mjGEOM_ARROW, NULL,NULL, NULL, yellow_color);
                  }
                  else
                  {
                      mjv_initGeom(particle_force, mjGEOM_ARROW, NULL,NULL, NULL, pink_color);
                  }
                  break;
                  ...
                  if(i>mcp_ep_visualization.particle_pos.at(j).size()-mcp_ep_visualization.exploit_particle_num.at(j))
                  {
                      mjv_initGeom(particle_force, mjGEOM_ARROW, NULL,NULL, NULL, cyon_color);
                  }
                  else
                  {
                      mjv_initGeom(particle_force, mjGEOM_ARROW, NULL,NULL, NULL, blue_color);
                  }
                  break;
                  ...
                  if(i>mcp_ep_visualization.particle_pos.at(j).size()-mcp_ep_visualization.exploit_particle_num.at(j))
                  {
                      mjv_initGeom(particle_force, mjGEOM_ARROW, NULL,NULL, NULL, white_green_color);
                  }

                  else
                  {
                      mjv_initGeom(particle_force, mjGEOM_ARROW, NULL,NULL, NULL, green_color);
                  }
                  break;
                  ...
                  if(i>mcp_ep_visualization.particle_pos.at(j).size()-mcp_ep_visualization.exploit_particle_num.at(j))
                  {
                      mjv_initGeom(particle_force, mjGEOM_ARROW, NULL,NULL, NULL, orange_color);
                  }
                  else
                  {
                      mjv_initGeom(particle_force, mjGEOM_ARROW, NULL,NULL, NULL, brown_color);
                  }
                  break;
                  ...
                  if(i>mcp_ep_visualization.particle_pos.at(j).size()-mcp_ep_visualization.exploit_particle_num.at(j))
                  {
                      mjv_initGeom(particle_force, mjGEOM_ARROW, NULL,NULL, NULL, gray_color);
                  }

                  else
                  {
                      mjv_initGeom(particle_force, mjGEOM_ARROW, NULL,NULL, NULL, white_color);
                  }
                  break;
              ...
              particle_force->size[0] = 0.0025;
              particle_force->size[1] = 0.0025;
              particle_force->size[2] = mcp_ep_visualization.particle_force.at(j).at(i).norm()*0.01;

              // particle_force->size[0] = 0.008;
              // particle_force->size[1] = 0.008;
              // particle_force->size[2] = visual_particle_force.at(j).at(i).norm()*0.015;

              // Eigen::Vector3d arrow_start = true_contact_point2 - true_contact_force2 * 0.005;
              Eigen::Vector3d arrow_start;
              arrow_start << mcp_ep_visualization.particle_pos.at(j).at(i)(0) - mcp_ep_visualization.particle_force.at(j).at(i)(0) * 0.005,
                              mcp_ep_visualization.particle_pos.at(j).at(i)(1) - mcp_ep_visualization.particle_force.at(j).at(i)(1) * 0.005,
                              mcp_ep_visualization.particle_pos.at(j).at(i)(2) - mcp_ep_visualization.particle_force.at(j).at(i)(2) * 0.005;

              // arrow_start << visual_particle_pos.at(j).at(i)(0) - visual_particle_force.at(j).at(i)(0) * 0.0075,
              //                visual_particle_pos.at(j).at(i)(1) - visual_particle_force.at(j).at(i)(1) * 0.0075,
              //                visual_particle_pos.at(j).at(i)(2) - visual_particle_force.at(j).at(i)(2) * 0.0075;


              particle_force->pos[0] = arrow_start(0);
              particle_force->pos[1] = arrow_start(1);
              particle_force->pos[2] = arrow_start(2);

              std::vector<double> particle_force_mat = EigenVec2Mat(mcp_ep_visualization.particle_force.at(j).at(i)/mcp_ep_visualization.particle_force.at(j).at(i).norm());

              for(int k=0; k<9; k++)
              {   
                  particle_force->mat[k] = particle_force_mat[k];
              }

          }
      }

      // For visualize representative particle
      for(int i=0; i<mcp_ep_visualization.points.size(); i++)
      {
          mjvGeom* represent_particle_force = scn.geoms + scn.ngeom++;
          switch(i)
          {
              case 0 :
              mjv_initGeom(represent_particle_force, mjGEOM_ARROW, NULL,NULL, NULL, deep_pink_color);
              break;
              
              case 1 :
              mjv_initGeom(represent_particle_force, mjGEOM_ARROW, NULL,NULL, NULL, deep_blue_color);
              break;

              case 2 :
              mjv_initGeom(represent_particle_force, mjGEOM_ARROW, NULL,NULL, NULL, deep_green_color);
              break;

              case 3 :
              mjv_initGeom(represent_particle_force, mjGEOM_ARROW, NULL,NULL, NULL, deep_brown_color);
              break;

              default : 
              mjv_initGeom(represent_particle_force, mjGEOM_ARROW, NULL,NULL, NULL, deep_white_color);
              break;
          }
          represent_particle_force->size[0] = 0.01;
          represent_particle_force->size[1] = 0.01;
          represent_particle_force->size[2] = mcp_ep_visualization.forces.at(i).norm()*0.01;

          Eigen::Vector3d arrow_start;
              arrow_start << mcp_ep_visualization.points.at(i)(0) - mcp_ep_visualization.forces.at(i)(0) * 0.005,
                              mcp_ep_visualization.points.at(i)(1) - mcp_ep_visualization.forces.at(i)(1) * 0.005,
                              mcp_ep_visualization.points.at(i)(2) - mcp_ep_visualization.forces.at(i)(2) * 0.005;

          represent_particle_force->pos[0] = arrow_start(0);
          represent_particle_force->pos[1] = arrow_start(1);
          represent_particle_force->pos[2] = arrow_start(2);

          std::vector<double> particle_force_mat = EigenVec2Mat(mcp_ep_visualization.forces.at(i)/mcp_ep_visualization.forces.at(i).norm());

          for(int k=0; k<9; k++)
          {   
              represent_particle_force->mat[k] = particle_force_mat[k];
          }
          
      }
    }

    if(mujoco_apply_force.is_visualize)
    {
      //For visualize true contact poinnt and force
      for(int i=0; i<mujoco_apply_force.contact_force.size(); i++)
      {
        mjvGeom* true_contact = scn.geoms + scn.ngeom++;
        mjv_initGeom(true_contact, mjGEOM_ARROW, NULL,NULL, NULL, deep_red_color);

        true_contact->size[0] = 0.01;
        true_contact->size[1] = 0.01;
        true_contact->size[2] = mujoco_apply_force.contact_force.at(i).norm()*0.01;

        Eigen::Vector3d arrow_start;
        arrow_start << mujoco_apply_force.contact_point.at(i)(0) - mujoco_apply_force.contact_force.at(i)(0) * 0.005,
                      mujoco_apply_force.contact_point.at(i)(1) - mujoco_apply_force.contact_force.at(i)(1) * 0.005,
                      mujoco_apply_force.contact_point.at(i)(2) - mujoco_apply_force.contact_force.at(i)(2) * 0.005;

        true_contact->pos[0] = arrow_start(0);
        true_contact->pos[1] = arrow_start(1);
        true_contact->pos[2] = arrow_start(2);

        std::vector<double> particle_force_mat = EigenVec2Mat(mujoco_apply_force.contact_force.at(i)/mujoco_apply_force.contact_force.at(i).norm());

        for(int k=0; k<9; k++)
        {   
            true_contact->mat[k] = particle_force_mat[k];
        }
      }
...
void Simulate::add_render(const MCP_EP_Visualization& mcp_ep_visualization_arg)
{  
  mcp_ep_visualization = mcp_ep_visualization_arg;
}

void Simulate::add_render(const MujocoApplyForce& mujoco_apply_force_arg)
{
